...
for i =1:m_iteration
    ...
    if i==1
    direction = -gradient;
    else direction=-gradient+gradient'*[gradient(1)-( -400*x(1,i-1)*(x(2,i-1)-x(1,i-1)^2) - 2*(1-x(1,i-1)));gradient(2)-(200*(x(2,i-1)-x(1,i-1)^2))]*direction/((-400*x(1,i-1)*(x(2,i-1)-x(1,i-1)^2)-2*(1-x(1,i-1)))^2+(200*(x(2,i-1)-x(1,i-1)^2))^2);
    end
